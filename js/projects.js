...
function startDocumentationBoot(){
    ...
    if(loadingLabel){
        ...
        loadingLabel.style.backgroundImage =
            "linear-gradient(90deg, #ff8a00 0%, #ff8a00 0%, #ffffff 0%, #ffffff 100%)";
    }
    let progress=0;
    ...
    function update(){
        ...
        if(loadingLabel){
            loadingLabel.style.backgroundImage =
                `linear-gradient(90deg, #ff8a00 0%, #ff8a00 ${progress}%, #ffffff ${progress}%, #ffffff 100%)`;
        }
    if(progress>=99){
...
function showDocumentationSystem(){
    ...
    buttons.forEach((button, index) => {
        button.classList.remove("show", "enter-left", "enter-right");
        button.classList.add(index % 2 === 0 ? "enter-left" : "enter-right");
        setTimeout(() => {
    ...
    buttons.forEach(button=>{
        button.onclick=()=>{
            ...
            if(documentationSystem){
                documentationSystem.classList.remove("popout-closed");
                documentationSystem.classList.add("popout-active");
            }
        };
    });
}
function buildModules(){
    ...
    if(!modules || Object.keys(modules).length===0){
        container.innerHTML=`
            <div class="technology-item empty-state">
...
function loadDocumentation(){
    ...
    const dataSource=getPortfolioData();
    const data=dataSource[currentCategory]?.modules?.[currentModule];
    if(!data){
        content.innerHTML=`
...
?
section.details.map(item=>`<li>${item}</li>`).join("")
:
